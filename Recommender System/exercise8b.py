...
def cofiCostFunc(params, Y, R, num_users, num_movies,
    ...
    for j in range(R.shape[1]):
        RatedIndeces = np.where(R[:, j] == 1)[0]
        Theta_grad[j, :] = np.dot(np.dot(X[RatedIndeces, :], Theta[j, :]) - Y[RatedIndeces, j], X[RatedIndeces, :]) + lambda_ * Theta[j, :]
    
    for i in range(R.shape[0]):
        RatedIndeces = np.where(R[i, :] == 1)[0]
        X_grad[i, :] = np.dot(np.dot(X[i, :], Theta[RatedIndeces, :].T) - Y[i, RatedIndeces], Theta[RatedIndeces, :]) + lambda_ * X[i, :]
